...
- 续写章节时，先获取该章节及前文上下文，再进行创作`,
  ...
  tools: {
    ...
    write: "统一写入工具，类似文件 Write：不传 id 时按 type 创建新实体；传 id 时整体覆盖该实体的全部可编辑字段（未提供的可选字段会被重置为默认值，如空字符串/空数组/importance=minor）。需要局部增量修改请改用 update_* 工具。worldId 和 projectId 在创建时自动注入；覆盖时不变更归属（worldId / projectId / scope 在覆盖模式下被忽略）。",
    write_type: "实体类型，决定其余字段的形态：character / world_setting / chapter / draft。",
    write_id: "实体 ID，可选。不传则创建；传则覆盖（整体替换可编辑字段，未提供的可选字段重置为默认值）。",

    write_character_name: "角色名称",
    write_character_importance: "重要性级别，默认 minor",
    write_character_summary: "一句话简介，不超过50字",
    write_character_aliases: "角色别名",
    write_character_tags: "角色标签，如：反派、魔法师、皇族等",
    write_character_content: "角色的详细描述（自然语言）。建议涵盖以下维度：外貌特征、性格特点、背景故事、目标动机、与其他角色的关系等。",
    write_character_scope: "角色归属范围（默认 world）。所有角色都会自动绑定当前 world。scope=world 时不写 projectId，对该世界下的所有小说可见，适合主角、共享反派、世界观核心人物等；scope=project 时同时写入当前 projectId，仅当前小说可见，适合本小说独有的配角、临时人物等不希望污染同世界其他小说的角色。",

    write_world_setting_category: "分类，如: 地理、历史、魔法体系",
    write_world_setting_title: "标题",
    write_world_setting_content: "内容",
    write_world_setting_tags: "标签",
    write_world_setting_importance: "重要性级别，默认 minor",
    write_world_setting_summary: "一句话简介，不超过50字",
    write_world_setting_scope: "设定归属范围（默认 world）。scope=world 时对该世界下的所有小说可见，适合通用世界观、共享背景；scope=project 时仅当前小说可见，适合本小说独有的设定细节、临时设定等。",

    write_chapter_title: "章节标题",
    write_chapter_content: "章节初始内容",
    write_chapter_synopsis: "章节梗概",
    write_chapter_order: "章节排序，不指定则自动排在最后",

    write_draft_title: "草稿标题",
    write_draft_content: "草稿内容",
    write_draft_tags: "标签",
    write_draft_linkedCharacters: "关联角色ID列表",
    write_draft_linkedWorldSettings: "关联世界观设定ID列表",
    write_draft_scope: "草稿归属范围（默认 world）。所有草稿都会自动绑定当前 world。scope=world 时不写 projectId，对该世界下的所有小说可见，适合写设定补充、世界观背景等共享信息；scope=project 时同时写入当前 projectId，仅当前小说可见，适合写本小说独有的伏笔、剧情转折、人物弧光等不希望污染同世界其他小说的内容。",
...
- When continuing a chapter, first retrieve the chapter and preceding context before writing`,
  ...
  tools: {
    ...
    write: "Unified write tool, modeled after a file Write: without id, creates a new entity of the given type; with id, overwrites every editable field of the existing entity (any optional field omitted is reset to its default — empty string, empty array, importance=minor, etc.). For partial in-place edits, use the update_* tools instead. worldId and projectId are auto-injected on create; on overwrite the entity's ownership (worldId / projectId / scope) is preserved and those parameters are ignored.",
    write_type: "Entity type — determines the rest of the field shape: character / world_setting / chapter / draft.",
    write_id: "Entity ID, optional. Omit to create; provide to overwrite (full replace of editable fields — omitted optional fields are reset to defaults).",

    write_character_name: "Character name",
    write_character_importance: "Importance level, default: minor",
    write_character_summary: "One-line summary, max 50 characters",
    write_character_aliases: "Character aliases",
    write_character_tags: "Character tags, e.g.: villain, mage, royalty",
    write_character_content: "Detailed character description in natural language. Recommended dimensions: appearance, personality, background story, goals/motivations, relationships with other characters.",
    write_character_scope: "Character ownership scope (defaults to world). Every character is bound to the current world automatically. scope=world stores no projectId and is visible to every novel under this world — use for protagonists, shared villains, or core world figures. scope=project also writes the current projectId so the character is visible only to the current novel — use for novel-specific side characters or transient figures that should not bleed into sibling novels.",

    write_world_setting_category: "Category, e.g.: Geography, History, Magic System",
    write_world_setting_title: "Title",
    write_world_setting_content: "Content",
    write_world_setting_tags: "Tags",
    write_world_setting_importance: "Importance level, default: minor",
    write_world_setting_summary: "One-line summary, max 50 characters",
    write_world_setting_scope: "Setting ownership scope (defaults to world). scope=world is visible to every novel under this world — use for shared lore, common background. scope=project is visible only to the current novel — use for novel-specific setting details or transient context that should not pollute sibling novels.",

    write_chapter_title: "Chapter title",
    write_chapter_content: "Initial chapter content",
    write_chapter_synopsis: "Chapter synopsis",
    write_chapter_order: "Chapter order; auto-placed at end if not specified",

    write_draft_title: "Draft title",
    write_draft_content: "Draft content",
    write_draft_tags: "Tags",
    write_draft_linkedCharacters: "Linked character ID list",
    write_draft_linkedWorldSettings: "Linked world setting ID list",
    write_draft_scope: "Draft ownership scope (defaults to world). Every draft is bound to the current world automatically. scope=world stores no projectId and is visible to every novel under this world — use for shared lore, background, or settings that apply to all stories. scope=project also writes the current projectId so the draft is visible only to the current novel — use for plot beats, foreshadowing, or character arcs that should not bleed into sibling novels in the same world.",
