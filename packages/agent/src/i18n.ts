...
- 续写章节时，先获取该章节及前文上下文，再进行创作`,
  ...
  tools: {
    ...
    write: "统一写入工具，类似文件 Write：不传 id 时按 type 创建新实体；传 id 时整体覆盖该实体的全部可编辑字段（未提供的可选字段会被重置为默认值，如空字符串/空数组/importance=minor）。需要局部增量修改请改用 update_* 工具。worldId 和 projectId 在创建时自动注入；覆盖时不变更归属（worldId / projectId / scope 在覆盖模式下被忽略）。各 type 的必填字段：character 必填 name；world_setting 必填 category + title；chapter 必填 title；draft 必填 title。",
    ...
    write_title: "标题：world_setting / chapter / draft 必填，character 不使用。",
    write_content: "正文内容：character=角色详细描述（外貌、性格、背景、动机、关系等）；world_setting=设定内容；chapter=章节正文；draft=草稿内容。",
    write_tags: "标签数组，character / world_setting / draft 适用，chapter 不使用。",
    write_importance: "重要性级别（core / major / minor，默认 minor），character / world_setting 适用。",
    write_summary: "一句话简介（不超过50字），character / world_setting 适用。",
    ...
    write_character_name: "角色名称（type=character 时必填）。",
    write_character_aliases: "角色别名数组（仅 character 使用）。",

    write_world_setting_category: "分类，如：地理、历史、魔法体系（type=world_setting 时必填）。",

    write_chapter_synopsis: "章节梗概（仅 chapter 使用）。",
    write_chapter_order: "章节排序（仅 chapter 使用）。创建时不指定则自动排在最后；覆盖时不指定则保持原顺序。",

    write_draft_linkedCharacters: "关联角色ID数组（仅 draft 使用）。",
    write_draft_linkedWorldSettings: "关联世界观设定ID数组（仅 draft 使用）。",
...
- When continuing a chapter, first retrieve the chapter and preceding context before writing`,
  ...
  tools: {
    ...
    write_title: "Title — required for world_setting / chapter / draft; not used by character.",
    ...
    write_tags: "Tags array — applies to character / world_setting / draft; not used by chapter.",
    write_importance: "Importance level (core / major / minor, default minor) — applies to character / world_setting.",
    write_summary: "One-line summary (max 50 chars) — applies to character / world_setting.",
    ...
    write_character_name: "Character name (required when type=character).",
    write_character_aliases: "Character aliases array (character only).",

    write_world_setting_category: "Category, e.g.: Geography, History, Magic System (required when type=world_setting).",

    write_chapter_synopsis: "Chapter synopsis (chapter only).",
    write_chapter_order: "Chapter order (chapter only). On create, auto-placed at end if not specified; on overwrite, preserves the existing order if not specified.",

    write_draft_linkedCharacters: "Linked character ID array (draft only).",
    write_draft_linkedWorldSettings: "Linked world setting ID array (draft only).",
