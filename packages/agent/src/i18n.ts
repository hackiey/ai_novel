...
- 续写章节时，先获取该章节及前文上下文，再进行创作`,
  ...
  tools: {
    ...
    update: "统一编辑工具，对实体的某个字段做局部修改。三种模式：1) 查找替换：传 old_string + new_string；2) 追加到末尾：传 append: true + new_string；3) 插入到开头：传 prepend: true + new_string。field 默认为 content。tags 字段以英文逗号分隔的字符串形式参与编辑（例如读到 \"悬疑,反派,核心\"），写回时自动按逗号拆分、去空白、去空项，存为数组。短字段（importance / summary）也按字符串编辑——例如 importance 用 old_string=\"minor\" + new_string=\"core\"。**重要约束**：character / world_setting 的 summary 是 worldSummary 的唯一数据源，每次用 update 改 content 后，**必须紧接着再调用一次 update field=summary 把 summary 同步到新 content**，否则 worldSummary 会漂移、污染后续会话上下文。",
    update_type: "实体类型：character（角色）/ world_setting（世界观设定）/ chapter（章节）/ draft（草稿）。",
    update_id: "实体 ID。",
    update_new_string: "新文本内容（替换文本 / 追加内容 / 插入内容）。field=tags 时按英文逗号分隔，例如 append 一个标签需传 \",新标签\"，包含分隔符。",
    update_old_string: "要被替换的原始文本片段（必须精确匹配）。查找替换模式下必填，append/prepend 模式下不需要。field=tags 时同样匹配逗号分隔字符串中的子串。",
    update_field: "目标字段，默认 content。各 type 可用字段：character = name / content / tags / importance / summary；world_setting = name / content / tags / importance / summary；chapter = name / content；draft = name / content / tags。name 在 character 写入 name 字段，在 world_setting / chapter / draft 写入 title 字段。",
    update_append: "设为 true 时，将 new_string 追加到字段末尾，无需 old_string。",
    update_prepend: "设为 true 时，将 new_string 插入到字段开头，无需 old_string。",

    write: "统一写入工具，类似文件 Write：不传 id 时按 type 创建新实体；传 id 时整体覆盖该实体本工具暴露的字段（name / summary / content / tags / importance 等，未提供的字段重置为默认值——空字符串 / 空数组 / importance=minor）。本工具不接触 UI 维护的字段（如章节梗概、章节顺序等），覆盖时这些字段保持不变。需要局部增量修改或编辑章节内容请改用 update 工具。worldId / projectId 在创建时自动注入；覆盖时不变更归属（worldId / projectId / scope 在覆盖模式下被忽略）。**必填字段**：所有 type 必填 name；character 和 world_setting 还必填 summary。**重要约束**：summary 是从 content 提炼的一句话简介，每次 write 时必须确保 summary 与 content 一致——只要 content 发生影响概要的变化，summary 就要同步更新（worldSummary 全靠 summary 构建，summary 漂移会污染所有后续会话的世界观摘要）。",
    write_type: "实体类型：character（角色）/ world_setting（世界观设定）/ chapter（章节）/ draft（草稿）。",
    write_id: "实体 ID，可选。不传则创建；传则覆盖（整体替换本工具暴露的字段；未在本工具暴露的字段保持不变）。",
    write_name: "实体名称 / 标题，所有 type 必填。character 写入 name 字段，world_setting / chapter / draft 写入 title 字段。",
    write_summary: "一句话简介（≤50 字），character 和 world_setting 必填。这是 worldSummary 构建的唯一数据源，必须高度提炼 content 的关键信息。每次 write 时务必确保 summary 与新写入的 content 保持一致——只要 content 发生有意义的变化，summary 就必须同步重写。chapter / draft 不使用此字段。",
    write_content: "正文内容：character=角色详细描述（外貌、性格、背景、动机、关系等）；world_setting=设定内容；chapter=章节正文；draft=草稿内容。",
    write_tags: "标签，英文逗号分隔的字符串（例如 \"悬疑,反派,核心\"），写入时自动按逗号拆分、去空白、去空项后存为数组。character / world_setting / draft 适用，chapter 不使用。",
    write_importance: "重要性级别（core / major / minor，默认 minor），character / world_setting 适用。",
    write_scope: "归属范围（world / project，默认 world），character / world_setting / draft 在创建时使用，chapter 不使用。scope=world 时对该世界下所有小说可见；scope=project 时仅当前小说可见。覆盖模式（传 id）下此字段被忽略。",
...
2. **世界观设定** — 提取明确描述的世界观设定，包括标题(title)、内容(content)、重要性(importance)、一句话简介(summary)
...
- When continuing a chapter, first retrieve the chapter and preceding context before writing`,
  ...
  tools: {
    ...
    update: "Unified edit tool that mutates a single field of an entity in place. Three modes: 1) Find-replace: pass old_string + new_string; 2) Append to end: pass append: true + new_string; 3) Prepend to start: pass prepend: true + new_string. Field defaults to content. The tags field is exposed as a comma-separated string for editing (e.g. you read \"mystery,villain,core\"); on write the tool splits by comma, trims whitespace, drops empties, and stores back as an array. Short fields (importance / summary) are also string-edited — e.g. set importance with old_string=\"minor\" + new_string=\"core\". **Critical**: for character / world_setting, summary is the only data source for worldSummary. Whenever you use update to change content, you MUST follow up with another update on field=summary so summary stays in sync with the new content — otherwise worldSummary drifts and pollutes every later session's context.",
    update_type: "Entity type: character / world_setting / chapter / draft.",
    update_id: "Entity ID.",
    update_new_string: "New text content (replacement / appended / prepended text). For field=tags use a comma-separated string — e.g. appending a tag requires \",newTag\" so the comma separator is included.",
    update_old_string: "Original text fragment to replace (must match exactly). Required for find-replace mode, not needed for append/prepend. For field=tags this matches a substring of the comma-joined tag string.",
    update_field: "Target field, defaults to content. Allowed per type — character: name / content / tags / importance / summary; world_setting: name / content / tags / importance / summary; chapter: name / content; draft: name / content / tags. `name` writes the `name` column for character and the `title` column for world_setting / chapter / draft.",
    update_append: "Set to true to append new_string to the end of the field. No old_string needed.",
    update_prepend: "Set to true to prepend new_string to the start of the field. No old_string needed.",

    write: "Unified write tool, modeled after a file Write: without id, creates a new entity of the given type; with id, overwrites the fields this tool exposes (name / summary / content / tags / importance, etc. — omitted fields are reset to defaults: empty string, empty array, importance=minor). This tool does NOT touch fields owned by other systems (chapter synopsis, chapter order, etc.) — those remain unchanged on overwrite. For partial in-place edits, use the update tool. worldId / projectId are auto-injected on create; on overwrite the entity's ownership (worldId / projectId / scope) is preserved and those parameters are ignored. **Required fields**: every type requires name; character and world_setting additionally require summary. **Critical**: summary is the one-line distillation of content. On every write you MUST keep summary consistent with the new content — if content changes meaningfully, summary must be rewritten to match (worldSummary is built entirely from these summary fields; drift here pollutes every future session's world overview).",
    write_type: "Entity type: character / world_setting / chapter / draft.",
    write_id: "Entity ID, optional. Omit to create; provide to overwrite (full replace of fields exposed by this tool; fields not exposed by this tool are preserved).",
    write_name: "Entity name / title — required for every type. Stored as `name` for character; stored as `title` for world_setting / chapter / draft.",
    write_summary: "One-line summary (≤50 chars) — required for character and world_setting. This is the only data source for the worldSummary that gets injected into every agent prompt, so it must be a tight distillation of `content`. On every write you MUST update summary in lockstep with content — if content changes meaningfully, rewrite summary to match. Not used by chapter / draft.",
    write_content: "Body content. character: detailed description (appearance, personality, background, motivations, relationships). world_setting: setting body. chapter: chapter text. draft: draft body.",
    write_tags: "Tags — a comma-separated string (e.g. \"mystery,villain,core\"). On write the value is split by comma, trimmed, emptied items dropped, and stored as an array. Applies to character / world_setting / draft; not used by chapter.",
    write_importance: "Importance level (core / major / minor, default minor) — applies to character / world_setting.",
    write_scope: "Ownership scope (world / project, default world) — applies on create for character / world_setting / draft; not used by chapter. scope=world is visible to every novel under this world; scope=project is visible only to the current novel. Ignored on overwrite (when id is provided).",
...
2. **World Settings** — Extract explicitly described world settings, including title, content, importance, and summary
